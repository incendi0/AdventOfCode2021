...
pub fn first_part(s: &str) -> i32 {
    ...
    for dx in 1..right_top.x {
        for dy in left_bottom.y..-left_bottom.y {
            ret = ret.max(height_with_speed(dx, dy, &left_bottom, &right_top).unwrap_or(left_bottom.y));
        }
    }
    ret
}
...
pub fn second_part(s: &str) -> i32 {
    ...
    for dx in 1..=right_top.x {
        for dy in left_bottom.y..=-left_bottom.y {
            ret += if shoots_to_area(dx, dy, &left_bottom, &right_top) {1} else {0};
        }
    }
    ret
}
...
fn height_with_speed(dx: i32, dy: i32, left_bottom: &Position, right_top: &Position) -> Option<i32> {
    let mut current_pos = Position { x: 0, y: 0 };
    let mut height = 0;
    let (mut dx, mut dy) = (dx, dy);
    let mut flag = false;
...
fn parse_position(s: &str) -> (Position, Position) {
    ...
    let xs: Vec<i32> = RE.find_iter(s)
        .filter_map(|digits| digits.as_str().parse().ok())
        .collect();
    println!("parse ret {:?}", xs);
    (Position { x: xs[0], y: xs[2] }, Position { x: xs[1], y: xs[3] })
}
